...
class SoftDarkStripeMask:
    """
    改进版遮罩：基于高斯模糊 + 分位数 + 局部平滑填充
    """
    def __init__(self, quantile=0.1, dilation_iter=5, blur_kernel=11):
        """
        quantile: 暗纹分位数阈值，例如 0.05 表示遮掉最暗的 5% 像素
        dilation_iter: 形态学膨胀的迭代次数，用于扩大遮罩
        blur_kernel: 高斯平滑的卷积核大小（必须是奇数）
        """
    ...
    def __call__(self, tensor):
        ...
        gray = tensor.squeeze(0)

        # 添加 batch 和 channel 维度
        gray_ = gray.unsqueeze(0).unsqueeze(0)

        # Step 1: 高斯模糊平滑
        blurred = TF.gaussian_blur(gray_, kernel_size=self.blur_kernel)

        # Step 2: 计算阈值并生成mask
        threshold = torch.quantile(blurred, self.quantile)
        mask = (blurred <= threshold).float()

        # Step 3: 形态学膨胀，扩展遮罩区域
        for _ in range(self.dilation_iter):
            mask = F.conv2d(mask, self.dilate_kernel, padding=1).clamp(0, 1)

        # Step 4: 平滑填充 — 使用原图的局部模糊图像代替暗条纹区域
        tensor_ = tensor.clone()
        blurred_full = TF.gaussian_blur(tensor_, kernel_size=self.blur_kernel)

        # mask 扩展到所有通道
        """if tensor_.ndim == 3:
            mask = mask.repeat(tensor_.shape[0], 1, 1)"""
        mask = mask.squeeze(0)
        # 平滑替换：在 mask 区域用 blurred 图像替换原图
        tensor_[mask.bool()] = blurred_full[mask.bool()]
...
class DataPreprocessor:
    ...
    def _build_transforms(self):
        ...
        if self.config.get('resize', {}).get('enabled', True):
         ...
         # ------------------- 新增功能 -------------------
        # 2.5. 将最小的 5% 像素设置为一个很小的值 (如果启用)
        if self.config.get('threshold_smallest', {}).get('enabled', True):
            quantile = self.config['threshold_smallest'].get('quantile', 0.10)
            set_value = self.config['threshold_smallest'].get('set_value', 1e-12)
            transforms_list.append(SoftDarkStripeMask())
            print(f"启用最小像素阈值处理: quantile={quantile}, set_value={set_value}")
...
class MultiTaskInterferenceDataset(Dataset):
    """
    多任务干涉数据集类
    - 同时进行回归任务（预测beta2, beta3）和分类任务（预测类别）
    """
...
def main():
    """主训练函数（GradNorm版本）"""
    # 加载配置
    with open('config_mask.yaml', 'r', encoding='utf-8') as f:
