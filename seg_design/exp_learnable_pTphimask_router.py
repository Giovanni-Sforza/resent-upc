...
class LearnableSectorGenerator(nn.Module):
    def __init__(self, H=224, W=224, num_regions=8):
        ...
        # --- 2. 初始化逻辑：全覆盖铺砖 (Tiling) ---
        
        # 我们有 8 个探针，做 2x4 布局
        # 内环: pT ~ 0.4 (覆盖 0.1-0.7)
        # 外环: pT ~ 0.9 (覆盖 0.6-1.2)
        
        # 设定中心位置
        # 前4个: 内环
        pt_inner = 0.4
        phi_num = int(num_regions/2+1)
        pt_num = int(num_regions/2)
        phi_inner = np.linspace(-np.pi, np.pi, phi_num)[:-1] # [-pi, -0.5pi, 0, 0.5pi]
        
        # 后4个: 外环 (角度错开 45度/0.25pi，填补空隙)
        pt_outer = 0.9
        phi_outer = phi_inner + (np.pi / 4) 
        
        # 合并
        init_mu_pt = np.concatenate([np.ones(pt_num)*pt_inner, np.ones(pt_num)*pt_outer])
        init_mu_phi = np.concatenate([phi_inner, phi_outer])
        
        # 转换为 Tensor Parameter
        self.mu_pt = nn.Parameter(torch.tensor(init_mu_pt, dtype=torch.float32))
        self.mu_phi = nn.Parameter(torch.tensor(init_mu_phi, dtype=torch.float32))
        
        # --- 3. 宽度初始化：大尺寸 ---
        
        # pT 宽度: 设为 0.3 左右。
        # 这样 Inner(0.4) ± 2*0.3 = [-0.2, 1.0]，完全覆盖低 pT
        # Outer(0.9) ± 2*0.3 = [0.3, 1.5]，完全覆盖高 pT
        # log(0.3) approx -1.2
        self.log_sigma_pt = nn.Parameter(torch.ones(num_regions) * -1.2)
        
        # phi 宽度: 设为 0.6 弧度 (~35度)。
        # 2*sigma ~ 70度。相邻探针间隔 90度。
        # 这样边缘会有适当重叠，保证没有死角。
        # log(0.6) approx -0.5
        self.log_sigma_phi = nn.Parameter(torch.ones(num_regions) * -0.5)

    def forward(self, batch_size):
        # ... (保持原本的扇形生成逻辑不变) ...
        # 注意：不要加 if self.training 的抖动(jitter)了
        # 或者把抖动设得非常小 (0.001)，因为现在是大范围覆盖，不需要靠抖动来探索
        ...
        # 限制最小宽度，防止坍缩成点
        sigma_phi = torch.exp(self.log_sigma_phi).view(self.num_regions, 1, 1) + 0.1 
...
class PhysicalDiscoveryModel(nn.Module):
    def __init__(self, config=None):
        ...
        # 1. 设定区域数量
        self.n_regions_skin = 2
        self.n_regions_def = 8
        self.n_regions = self.n_regions_skin + self.n_regions_def # 总共8个
        
        self.obs_dim_per_region = 41 
        
        # 2. 生成器 (保持不变，生成8个区域)
        #self.region_generator = LearnableSectorGenerator(
        ...
        # Skin Head 只看前 3 个区域
        dim_skin = self.n_regions_skin * self.obs_dim_per_region
        self.norm_skin = nn.BatchNorm1d(dim_skin)
        self.head_skin = TaskHead(dim_skin, 3) 
        
        # Def Head 只看后 5 个区域
        dim_def = self.n_regions_def * self.obs_dim_per_region
        self.norm_def = nn.BatchNorm1d(dim_def)
        self.head_def = TaskHead(dim_def, 2) 
    ...
    def forward(self, x):
        ...
        # 切片操作：前3个给 Skin
        feats_skin = all_feats[:, :self.n_regions_skin, :] # (B, 3, 16)
        
        # 切片操作：后5个给 Def
        feats_def  = all_feats[:, self.n_regions_skin:, :] # (B, 5, 16)
        
        # ===================================================
        
        # 3. Flatten
        flat_skin = feats_skin.reshape(B, -1) # (B, 48)
        flat_def  = feats_def.reshape(B, -1)  # (B, 80)
        
        # 4. 预测
        out_skin = self.head_skin(self.norm_skin(flat_skin))
        out_def  = self.head_def(self.norm_def(flat_def))
        
        # 返回 regions 方便画图验证位置
        return out_def, out_skin#, regions
