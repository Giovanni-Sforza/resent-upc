...
# ==========================================
# 模块 A: 启发式分割器 (Heuristic Segmenter)
# ==========================================
class HeuristicSegmenter(nn.Module):
    """
    不含可学习参数。基于像素强度百分位生成 3 个 Mask。
    """
    def __init__(self, bright_p=0.5, dark_p=0.4):
        super().__init__()
        self.bright_p = bright_p # Top 30%
        self.dark_p = dark_p     # Bottom 10% (excluding 0)

    def forward(self, x):
        # x: (B, 3, H, W) -> 取平均变成灰度 (B, 1, H, W)
        img = x.mean(dim=1, keepdim=True)
        B, C, H, W = img.shape
        
        # 展平以便计算百分位 (B, H*W)
        flat_img = img.view(B, -1)
        
        # 1. 计算阈值
        # kthvalue 返回的是从小到大排序的第 k 个值
        # Bright: > 70% -> k = 0.7 * Total
        k_bright = int(self.bright_p * H * W)
        thresh_bright = torch.kthvalue(flat_img, k_bright, dim=1).values.view(B, 1, 1, 1)
        
        # Dark: < 10% -> k = 0.1 * Total
        k_dark = int(self.dark_p * H * W)
        thresh_dark = torch.kthvalue(flat_img, k_dark, dim=1).values.view(B, 1, 1, 1)
        
        # 2. 生成 Masks (Binary)
        # Mask 0: Bright (High pT region)
        mask_bright = (img > thresh_bright).float()
        
        # Mask 1: Dark/Fringe (Interference region)
        # 注意：通常要排除纯黑背景(0值)，这里假设 preprocess 后的数据有底噪或 LogNorm
        mask_dark = (img < thresh_dark).float()
        
        # Mask 2: Noise/Background (The rest)
        mask_noise = 1.0 - mask_bright - mask_dark
        # 确保数值稳定性（处理边界情况）
        mask_noise = torch.clamp(mask_noise, 0.0, 1.0)

        # Output: (B, 3, H, W)
        masks = torch.cat([mask_bright, mask_dark, mask_noise], dim=1)
        return masks.detach() # 启发式分割不需要梯度
...
class PhysicsOperatorAdapter(nn.Module):
    """
    [V3.0 Final] 形状矩适配器 (Moment-based Adapter)
    专门针对: 低 pT (<0.17 GeV), 低统计量 (Low Stats), 噪声敏感场景。
    核心思想: 不分箱(Binning)，只做全图积分(Integration)以抑制噪声。
    """
    def __init__(self):
        super().__init__()
        
        # Buffer 占位
        self.register_buffer('pt_map', None)
        self.register_buffer('cos2', None)
        self.register_buffer('sin2', None)
        self.register_buffer('cos3', None)
        self.register_buffer('sin3', None)

    def _init_grid(self, H, W, device):
        if self.pt_map is None:
            # 假设输入图片被crop在 pT < 0.17 GeV 范围内
            # 这里归一化坐标到 [-1, 1] 对应 [-0.17, 0.17]
            ...
            self.pt_map = pt.unsqueeze(0).unsqueeze(0) # (1,1,H,W)
            self.cos2 = torch.cos(2*phi).unsqueeze(0).unsqueeze(0)
            self.sin2 = torch.sin(2*phi).unsqueeze(0).unsqueeze(0)
            self.cos3 = torch.cos(3*phi).unsqueeze(0).unsqueeze(0)
            self.sin3 = torch.sin(3*phi).unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, img, masks):
        ...
        self._init_grid(H, W, masks.device)
        if img.shape[1] == 3:
            # 如果是 RGB，取平均变成单通道
        ...
        for t in range(N_tasks):
            mask = masks[:, t:t+1]
            w_img = img * mask # 加权图像 (Effective Counts)
            
            # --- 0. 基础量 ---
            total_yield = w_img.sum(dim=(2,3)) + 1e-9
            
            # --- 1. 径向形状 (Radial Shape) - 针对 Skin Task ---
            
            # <pT> (一阶矩)
            mean_pt = (w_img * self.pt_map).sum(dim=(2,3)) / total_yield
            
            # <pT^2> (二阶矩, 关联温度/宽度)
            mean_pt2 = (w_img * self.pt_map**2).sum(dim=(2,3)) / total_yield
            
            # <pT^4> (四阶矩) - 用于计算峰度
            mean_pt4 = (w_img * self.pt_map**4).sum(dim=(2,3)) / total_yield
            
            # Radial Kurtosis (峰度): 
            # 如果主要特征是衍射导致的“平顶”或“凹陷”，峰度会显著变小
            # 为了数值稳定性，取 log 或简单比值
            # Gaussian 的 pt4/pt2^2 = 2 (2D case). Hollow shape < 2.
            kurtosis = mean_pt4 / (mean_pt2**2 + 1e-6)
            
            # Hollow Score (空心度)
            # 简单粗暴：内圈 (pT < 0.5) 的占比
            # 0.5 是相对值，对应 pT grid 的一半
            inner_mask = (self.pt_map < 0.5).float()
            inner_yield = (w_img * inner_mask).sum(dim=(2,3))
            hollow_ratio = inner_yield / total_yield

            # --- 2. 各向异性 (Anisotropy) - 针对 Deformation Task ---
            
            # v2 Vector
            q2_x = (w_img * self.cos2).sum(dim=(2,3))
            q2_y = (w_img * self.sin2).sum(dim=(2,3))
            v2_mag = torch.sqrt(q2_x**2 + q2_y**2) / total_yield
            
            # v3 Vector (Triangularity)
            q3_x = (w_img * self.cos3).sum(dim=(2,3))
            q3_y = (w_img * self.sin3).sum(dim=(2,3))
            v3_mag = torch.sqrt(q3_x**2 + q3_y**2) / total_yield
            
            # Mean-pT weighted v2 (硬 v2)
            # <pT * cos2phi>
            # 这个量对边缘的形变非常敏感，且不受中心背景噪声影响
            q2pt_x = (w_img * self.pt_map * self.cos2).sum(dim=(2,3))
            q2pt_y = (w_img * self.pt_map * self.sin2).sum(dim=(2,3))
            # 归一化通常除以 <pT> * Yield
            v2_pt_mag = torch.sqrt(q2pt_x**2 + q2pt_y**2) / (total_yield * mean_pt + 1e-9)

            # --- 组装特征 (Dim = 8) ---
            task_feat = torch.cat([
                torch.log(total_yield + 1), # [0] Log Multiplicity
                mean_pt,                    # [1] Mean pT
                torch.sqrt(mean_pt2),       # [2] RMS pT
                kurtosis,                   # [3] Shape: Peaked vs Flat (Critical for Skin)
                hollow_ratio,               # [4] Shape: Dip check (Critical for Skin)
                v2_mag,                     # [5] v2
                v3_mag,                     # [6] v3
                v2_pt_mag                   # [7] Hard v2 (Critical for Def)
            ], dim=1)
...
# ==========================================
class PhysicalRoutingModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        # 1. 启发式分割 (Static)
        ...
        # 2. 路由 (Learnable)
        self.router = GumbelSinkhornRouter(num_masks=3, num_tasks=3, init_temp=1.0)
        
        # 3. 物理算子 (Static)
        obs_dim = 16
        self.adapter = LearnablePhysicsAdapter(num_new_operators=obs_dim)#PhysicsOperatorAdapter()
        
       
        # 4. 任务头 (Linear Probes)
        self.feature_norm = nn.BatchNorm1d(obs_dim)
        # 物理算子维度 = 4
        # Task 0: Skin (3 Classes) -> 对应 Bright Mask
        #self.head_skin = nn.Linear(obs_dim, 3, bias=False)
        self.head_skin = TaskHead(obs_dim, 3)
        # Task 1: Deformation (2 Regressions) -> 对应 Dark Mask
        #self.head_def = nn.Linear(obs_dim, 2, bias=False)
        self.head_def = TaskHead(obs_dim, 2)
        # Task 2: Trash -> 不需要 Head，或者用一个 Dummy
        
        # 初始化 Linear 层为正交矩阵或较小随机数，方便观察权重
        #nn.init.orthogonal_(self.head_skin.weight)
        #nn.init.orthogonal_(self.head_def.weight)
    ...
    def forward(self, x, hard_routing=False):
        """
        Standard forward for training loop.
        """
        # A. Segment
        raw_masks = self.segmenter(x) # (B, 3, H, W)
        
        # B. Route
        routed_masks, P = self.router(raw_masks, hard=hard_routing)
        
        # C. Physics Ops
        # feats: (B, 3, 4)
        # Channel 0 -> Assigned to Skin
        # Channel 1 -> Assigned to Def
        # Channel 2 -> Assigned to Trash
        feats = self.adapter(x, routed_masks)

        B, n_tasks, n_feats = feats.shape
        feats_flat = feats.view(B * n_tasks, n_feats)
        feats_norm = self.feature_norm(feats_flat)
        feats = feats_norm.view(B, n_tasks, n_feats)
        # D. Heads
        # 取出对应的通道特征
        feat_skin = feats[:, 0, :] # (B, 4)
        feat_def  = feats[:, 1, :] # (B, 4)
        
        # 预测
        out_skin = self.head_skin(feat_skin) # (B, 3)
        out_def  = self.head_def(feat_def)   # (B, 2)
        
        # 增加一个属性，方便在 evaluate 时提取 P
        self.last_P = P.detach()
        self.last_routed_masks = routed_masks.detach()
        
        # 必须返回 (reg, cls) 以兼容原有训练循环
        # 注意：out_def 是 beta2, beta3
        return out_def, out_skin



# ==========================================
# 2. 全图验证模型 (Sanity Check Model)
# ==========================================
class PhysicalSanityCheckModel(nn.Module):
    def __init__(self, config=None):
        ...
        # --- 移除路由器 ---
        # self.router = GumbelSinkhornRouter(...)
        
        # --- 物理算子 (保留) ---
        # 建议使用之前讨论的 LearnablePhysicsOperatorAdapter 或 PhysicsOperatorAdapter()
        obs_dim = 16 
        self.adapter = LearnablePhysicsOperatorAdapter(num_learnable_features=obs_dim)#PhysicsOperatorAdapter()
        
        # --- 任务头 (升级为 Robust MLP) ---
        self.feature_norm = nn.BatchNorm1d(obs_dim)
        
        # Task 0: Skin (3 Classes)
        self.head_skin = TaskHead(obs_dim, 3)
        
        # Task 1: Deformation (2 Regressions)
        self.head_def = TaskHead(obs_dim, 2)
        
        # 记录一下不需要梯度的属性，防止训练脚本报错
        self.register_buffer('last_P', torch.zeros(1)) 

    def forward(self, x, hard_routing=False):
        """
        强制全图模式：
        跳过 Segmentation 和 Routing，直接让 Task Head 看整张图的物理特征。
        """
        # 1. 强制单通道处理 (防止之前的 Channel 报错)
        if x.shape[1] == 3:
            x = x.mean(dim=1, keepdim=True)
            
        B, C, H, W = x.shape
        
        # 2. 创建“全图 Mask” (Global Mask)
        # 我们生成 2 个通道的 Mask，分别对应 Skin 和 Def 任务
        # 此时 mask 的值全为 1，意味着算子对整张图进行积分
        global_masks = torch.ones((B, 2, H, W), device=x.device)
        
        # 3. 计算物理算子
        # feats: (B, 2, obs_dim)
        # 因为输入 Mask 是一样的，LearnableAdapter 可能会输出两组一样的特征，
        # 但没关系，我们主要看后面的 MLP Head 能不能从这组特征里提取出不同的东西。
        feats = self.adapter(x, global_masks)
        
        # 4. 归一化 (这一步在 RobustHead 里也有 BN，这里做一次也无妨)
        B, n_tasks, n_feats = feats.shape
        feats_flat = feats.view(B * n_tasks, n_feats)
        feats_norm = self.feature_norm(feats_flat)
        feats = feats_norm.view(B, n_tasks, n_feats)
        
        # 5. 训练阶段加入噪声 (Feature Jittering) - 进一步抗过拟合
        if self.training:
            noise = torch.randn_like(feats) * 0.05 * feats.detach() # 5% 的相对噪声
            feats = feats + noise
        ...
        out_skin = self.head_skin(feat_skin)
        out_def  = self.head_def(feat_def)
...
def create_model(config, device='cpu'):
    ...
    elif mode == 'physical_routing_poc': # 新增模式
        return PhysicalSanityCheckModel(config)
    else:
